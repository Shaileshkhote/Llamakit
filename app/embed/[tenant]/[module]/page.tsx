...
export default async function EmbedPage({
  ...
  const [{ tenant: tenantParam, module }, { theme: themeParam }] = await Promise.all([params, searchParams])
  const tenant = await getTenantBySlugOrHost(decodeURIComponent(tenantParam))
  const metricKey = moduleToMetric[module]

  if (!tenant || !tenant.published || !metricKey) notFound()
  ...
  return (
    ...
    >
      <div className="mb-2 flex justify-between gap-2">
        <strong>
          {tenant.displayName} {metric.label}
        </strong>
        <span className={`text-xs ${dark ? "text-[#aab7c8]" : "text-[#5c6b7d]"}`}>
          {formatTimestamp(metric.lastDataAt)}
        </span>
      </div>
      <EmbedChart metric={metric} accentColor={tenant.accentColor} theme={theme} />
